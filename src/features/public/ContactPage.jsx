...
export default function ContactPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setError('')
    if (!form.name.trim() || form.name.trim().length < 2) return setError(t('contact.err.name'))
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) return setError(t('contact.err.email'))
    if (!form.serviceNeeded) return setError(t('contact.err.service'))
    if (!form.message.trim() || form.message.trim().length < 10) return setError(t('contact.err.message'))
        ...
        <div className="grid md:grid-cols-2 gap-16">
          ...
          <div>
            ...
            <div className="mt-10 space-y-5">
              ...
              <InfoRow icon={MapPin} title={t('contact.location')}>{t('contact.locationVal')}</InfoRow>
              <InfoRow icon={Clock} title={t('contact.response')}>{t('contact.responseVal')}</InfoRow>
            </div>
          </div>
          ...
          <div>
            ...
            <form className="space-y-4" onSubmit={handleSubmit} noValidate>
                <input type="text" name="website_url" value={form.website_url}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label={t('contact.f.name')} required>
                    <Input value={form.name} onChange={e => update('name', e.target.value)} placeholder={t('contact.f.namePh')} autoComplete="name" disabled={loading} />
                  </Field>
                  <Field label={t('contact.f.email')} required>
                    <Input type="email" value={form.email} onChange={e => update('email', e.target.value)} placeholder={t('contact.f.emailPh')} autoComplete="email" disabled={loading} />
                  </Field>
                </div>
                <Field label={t('contact.f.business')}>
                  <Input value={form.businessName} onChange={e => update('businessName', e.target.value)} placeholder={t('contact.f.businessPh')} autoComplete="organization" disabled={loading} />
                </Field>
                <Field label={t('contact.f.current')}>
                  <Input type="url" value={form.currentWebsite} onChange={e => update('currentWebsite', e.target.value)} placeholder={t('contact.f.currentPh')} autoComplete="url" disabled={loading} />
                </Field>
                <Field label={t('contact.f.need')} required>
                ...
                <Field label={t('contact.f.message')} required>
                  <textarea
                ...
                <p className="text-[11px] text-text-tertiary text-center leading-relaxed">
                  {t('contact.f.foot1')} <span className="text-text-secondary">{t('contact.f.foot2')}</span> — {t('contact.f.foot3')}<br/>
                  {t('contact.f.direct')} <a href="mailto:general@cloz.digital" className="text-accent hover:text-accent-hover">general@cloz.digital</a>
                </p>
              </form>
          </div>
        </div>
