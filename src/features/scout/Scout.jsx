...
export default function Scout() {
  ...
  }, [filterStatus, filterNoWebsite, sort, searchQuery])

  const loadStats = async () => { try { setStats(await scout.stats()) } catch {} }

  useEffect(() => { loadLeads() }, [filterStatus, filterNoWebsite, sort, searchQuery])
      ...
      <div className={`${selectedLead ? 'w-[440px]' : 'flex-1 max-w-[960px]'} flex flex-col border-r border-border bg-bg overflow-hidden transition-all`}>
        ...
        {activeTab === 'leads' && (
          <>
            ...
            {/* Lead list */}
            <LeadList leads={leads} loading={loading} selectedId={selectedLead?.id} onSelect={setSelectedLead} />
          </>
        )}
        ...
        <button key={lead.id} onClick={() => onSelect(lead)}
          ...
          <div className="flex items-start justify-between mb-0.5">
            <div className="flex items-center gap-2 min-w-0">
              ...
              {lead.place_id && <MapPinned size={10} className="text-success shrink-0" title="Google verified" />}
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            {lead.place_id && <MapPinned size={14} className="text-success" title="Google verified" />}
            {lead.opportunity_score > 0 && (
